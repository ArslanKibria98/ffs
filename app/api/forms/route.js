  ...
  try {
    ...
    if (!email && !formId) {
      return NextResponse.json(
        { error: 'Email or FormID is required!' },
        { status: 500 }
      )
    }
...
export async function POST(req, res) {
  try {
    ...
    let updatedForm
    const existingForm =
      existingUser?.forms?.find(
        (form) => form.name == JSON.parse(formData?.formName)
      ) ?? null
...
export async function DELETE(req) {
  try {
    ...
    if (!formId || formId == '') {
      return NextResponse.json({ error: 'Form ID missing!' }, { status: 500 })
    }
