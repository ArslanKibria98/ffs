...
export default function Home() {
  ...
  const toggleFormStatus = async (id, status) => {
    try {
      const response = await axios.get(
        `/api/forms/status-change?formId=${id}&email=${session.user.email}&status=${status}`
      )
  ...
  const copyFormApi = async (id, status) => {
    let apiString
    apiString =
      'http://localhost:3000/api/forms?formId=' + id + '&public=' + status
        ...
        <AlertDialog>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                <input
                  ...
                  className="p-2 my-2 border rounded-lg w-full"
                  required
                />
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              ...
              <AlertDialogAction
                onClick={() => createForm(session?.user?.email, formDataInit)}
                className="bg-red-500 hover:bg-red-600"
                disabled={
                  !newFormName || newFormName == '' || newFormName.length < 4
                    ? true
                    : false
                }
              >
              ...
              return (
                <HoverCard key={index}>
                  <HoverCardTrigger href={`/home/form/${item.id}`}>
                    ...
                    <div className="flex flex-col justify-between items-center py-2 text-left bg-red-400 h-[180px] duration-200 rounded-xl shadow-lg hover:shadow-xl shadow-gray-300 hover:shadow-gray-400 text-red-100 hover:text-white">
                      <div className="w-full px-2">
                        ...
                        <div className="py-2">{parsedContent?.createdAt}</div>
                        <div>
                          <strong>Form Type:</strong>{' '}
                          {parsedContent?.multi_step
                            ? 'Multi Step'
                            : 'Single Step'}
                        </div>
                      </div>
                      <p className="text-center font-extrabold pb-4">
                        {parsedContent?.formName}
                  ...
                  <HoverCardContent className="w-fit flex gap-3">
                    <button
                      onClick={() => copyFormApi(item.id, item.type)}
                      className="p-2 border-0 rounded text-yellow-500 hover:text-white bg-white hover:bg-yellow-500 duration-200"
                    >
                      <Copy />
                    </button>
                    {item.type == 'false' || item.type == false ? (
                      <button
                        onClick={() => toggleFormStatus(item.id, true)}
                        className="p-2 border-0 rounded text-green-500 hover:text-white bg-white hover:bg-[green] duration-200"
                      >
                        <svg
                      ...
                      <button
                        onClick={() => toggleFormStatus(item.id, false)}
                        className="p-2 border-0 rounded text-orange-500 hover:text-white bg-white hover:bg-[orange] duration-200"
                      >
                        <LockKeyholeIcon />
                      </button>
                    )}
                    <button
                      ...
                      className="p-2 border-0 rounded text-red-500 hover:text-white bg-white hover:bg-[red] duration-200"
                    >
                      <Trash2 />
                    </button>
                  </HoverCardContent>
