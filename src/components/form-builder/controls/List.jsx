...
export default function List({
  ...
  const [radioHeaders, setRadioHeaders] = useState([
    { parmKey: "", parmValue: "" },
  ]);
  ...
  const [choices, setChoices] = useState(
    !isUpdate
      ? ["", ""]
      : updateFieldData?.choices?.map((choice) => choice?.value)
  );
  ...
  return (
    <div>
      <DialogTitle>{`${isUpdate?"Update":"Add"}`} List</DialogTitle>
      <br />
      <RadioGroup
        ...
        <>
          <div className="grid grid-cols-2 gap-8 gap-y-3">
            ...
            <div className="col-span-2">
              ...
              <Select
                className="w-full"
                onValueChange={(e) => setFormatData(e)}
          ...
          <Select
            className="w-full"
            onValueChange={(e) => setSelectionData(e)}
            defaultValue={selectionData}
          >
            <SelectTrigger className="w-full">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {selection.map((size, index) => (
...
{selectionData&&
        <div>
          ...
          <Select
            className="w-full"
            onValueChange={(e) => setSelectionDataType(e)}
            defaultValue={selectionDataType}
          >
            <SelectTrigger className="w-full">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {selectionType.map((style, index) => (
                <SelectItem key={index} value={style.value}>
                  {style.label}
                </SelectItem>
              ))}
